...
window.addEventListener('load', () => {
  ...
  tl.to('#ldLogo', { opacity: 1, y: 0, duration: 0.7, ease: 'power3.out' })
    .to('#ldFill',  { width: '100%', duration: 1.6, ease: 'power2.inOut' }, '-=0.3')
    .to('#loader',  {
      yPercent: -100,
      duration: 0.85,
      ease: 'power3.inOut',
...
──────────────────────────── */
function initHero() {
  gsap.to('#hBadge', { opacity: 1, y: 0, duration: 0.8, delay: 0.05, ease: 'power3.out' });
  gsap.to('#hTitle', { opacity: 1, y: 0, duration: 0.9, delay: 0.22, ease: 'power3.out' });
  gsap.to('#hSub',   { opacity: 1, y: 0, duration: 0.8, delay: 0.40, ease: 'power3.out' });
  gsap.to('#hBtns',  { opacity: 1, y: 0, duration: 0.8, delay: 0.56, ease: 'power3.out' });
  gsap.to('#hScroll',{ opacity: 1, duration: 0.7, delay: 0.90, ease: 'power2.out' });

  // Stagger fact strip items
  gsap.to('.fact-item', {
    opacity: 1, y: 0,
    duration: 0.6, stagger: 0.08, delay: 0.7, ease: 'power3.out'
  });
}
...
function updateNav() {
  const pastHero = window.scrollY >= window.innerHeight * 0.80;
...
function navScrollTo(id) {
  ...
  const landingActive = document.getElementById('landingPg').classList.contains('active');
